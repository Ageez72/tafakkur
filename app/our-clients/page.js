...
export default function TafakuurInstitutions() {
    ...
    const handleCountryFilter = (countryId)=>{
        ...
        const fetchData = async () => {
            try {
                const res = await fetch(`${state.HTTP_URL}schools?country_id=${countryId}&limit=${limit}&page=${page}`, requestOptions);
                if (!res.ok) {
    ...
    const handlePaginationFilter = (pageNumber) => {
        ...
        const fetchData = async () => {
            try {
                const res = await fetch(`${state.HTTP_URL}schools?country_id=${country}&limit=${limit}&page=${pageNumber}`, requestOptions);
                if (!res.ok) {
                    throw new Error("Failed to fetch data");
                }
                const jsonData = await res.json();
        ...
        <>
            ...
            >
                <section className="tafakuur-institutions top">
                    ...
                    <CountryFilterTable title="اختر الدولة" data={institutionsData?.schools?.data} links={institutionsData?.schools?.links} countries={countriesList} handleSubmit={handleCountryFilter} handlePagination={handlePaginationFilter}/>
                            ...
                            <div className="container custom-container">
                                ...
                                <div className="row h-100">
                                    ...
                                    <div className="col-12 col-lg-6">
                                        <AnosTestimonialSlider cols={1} data={institutionsData?.testimonials}/>
                                    </div>
                                </div>
                            </div>
                        </div>
